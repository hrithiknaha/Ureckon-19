  ...
  ************************************************************************/
  ...
  PolluxFx.prototype._in = function () {
    ...
    anime({
      ...
      duration: 1000,
      opacity: {
        value: function value(target, index, cnt) {
          return index !== cnt - 1 ? [0, 0.1 * index + 0.1] : 1;
        },
        easing: 'linear',
        delay: function delay(target, index, cnt) {
          return (cnt - index - 1) * 60;
        }
      },
      translateY: {
        value: function value(target, index) {
          return -1 * index * 10;
        },
...
var time_to_recreate = false; // Enable repopolate

setTimeout(function () {
  time_to_recreate = true;
}.bind(void 0), max_time); // Popolate particles
